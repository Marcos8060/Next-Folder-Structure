...
export default async function handler(req, res) {
    if (req.method === API_METHODS.GET) {
        try {
            ...
            await backendAxiosInstance.get(`${API_URL.GET_PERMISSIONS}`, config).then(response => {
        ...
        try {
            ...
            await backendAxiosInstance.post(`${API_URL.ADD_ROLE_PERMISSION}/${body.roleId}`,body.permissions, config)
                .then(response => {
                    res.status(200).json(response.data);
                })
                .catch(e => {
