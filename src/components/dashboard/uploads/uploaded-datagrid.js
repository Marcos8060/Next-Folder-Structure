...
import { getAllFileUploads } from "../../../slices/upload";
...
const UploadedDatagrid = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values,helpers) => {
            try {
                await dispatch(getAllFileUploads(authUser, values));
                helpers.resetForm();
    ...
    const action = ({ displayValue,data}) =>{

        const handleExportExcel = async () => {
            try{
                await download(displayValue, data?.file)
            }
            catch (e) {
